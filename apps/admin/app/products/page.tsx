...
export default function ProductsPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (
      hasPriceInput &&
      (!numericPrice || !isFinite(numericPrice) || numericPrice <= 0)
    ) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!allowedTypes.includes(type)) {
        throw new Error(
          "Noto‘g‘ri fayl turi. Ruxsat etilgan: JPEG, PNG, WebP, GIF, MP4, WebM, MOV.",
        );
      }
          ...
          <div className="flex items-center gap-2 pb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Webapp katalogi uchun mahsulotlarni qo‘shish va boshqarish
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleCreate} className="space-y-4 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Rasm yoki video (URL yoki yuklash)
                </label>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Rasm: JPEG, PNG, WebP, GIF. Video: MP4, WebM, MOV. Maks. 100
                  MB.
                </p>
                {uploadError && (
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Aksiya badji (ixtiyoriy)
                </label>
                <Input
                  value={badgeLabel}
                  onChange={(e) => setBadgeLabel(e.target.value)}
                  placeholder="Masalan: Aksiya, Chegirma"
                />
              </div>
            </div>
            <div>
                      ...
                      <TableRow key={String(p._id)}>
                        ...
                        <TableCell className="text-right">
                          <Button
                            variant="destructive"
                            size="sm"
                            onClick={() => handleDelete(p._id as string)}
                          >
                            O‘chirish
                          </Button>
                        </TableCell>
